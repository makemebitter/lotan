...
object GraphGenerators extends Logging {
    ...
    /** Generate a graph whose vertex out degree distribution is log normal.
      ...
      * for large-scale graph processing. SIGMOD '10.
      *
      * If the seed is -1 (default), a random seed is chosen. Otherwise, use the
      * user-specified seed.
      *
      * @param sc
      *   Spark Context
      ...
      * @param seed
      *   (optional, default: -1) seed for RNGs, -1 causes a random seed to be
      *   chosen
      * @return
      *   Graph object
      */
    ...
    /** Randomly samples from the given mean and standard deviation of the
      * normal distribution. It uses the formula `X = exp(mu+sigma*Z)` where
      * `mu`, `sigma` are the mean, standard deviation of the normal
      * distribution and `Z ~ N(0, 1)`.
      *
      * @param mu
      ...
      * @param maxVal
      *   exclusive upper bound on the value of the sample
      * @param seed
      *   optional seed
      */
    private def sampleLogNormal(
        mu: Double,
        sigma: Double,
        maxVal: Int,
        seed: Long = -1
    ...
    /** A random graph generator using the R-MAT model, proposed in "R-MAT: A
    ...
    def rmatGraph(
        ...
        val numEdgesUpperBound =
            math.pow(2.0, 2 * ((math.log(numVertices) / math.log(2.0)) - 1))
                .toInt
    ...
    /** @param numVertices
      *   Specifies the total number of vertices in the graph (used to get the
    ...
    /** This method recursively subdivides the adjacency matrix into quadrants
      ...
      * graph. The adjacency matrix looks like: <pre>
      *
      * dst -> (x,y) *************** _
      * |   |   |  |
      * |:--|:--|:-|
      ...
      * subdivided. (x,y) represent the upper left hand corner of the
      * subquadrant, and T represents the side length (guaranteed to be a power
      * of 2).
      *
      * After choosing the next level subquadrant, we get the resulting sets of
      * parameters:
      * {{{
    ...
    /** Create `rows` by `cols` grid graph with each vertex connected to its
      * row+1 and col+1 neighbors. Vertex ids are assigned in row major order.
      *
      * @param sc
      ...
      *
      * @return
      *   A graph containing vertices with the row and column ids as their
      *   attributes and edge values as 1.0.
      */
    def gridGraph(
        sc: SparkContext,
        rows: Int,
    ...
    /** Create a star graph with vertex 0 being the center.
      ...
      *
      * @return
      *   A star graph containing `nverts` vertices with vertex 0 being the
      *   center vertex.
      */
